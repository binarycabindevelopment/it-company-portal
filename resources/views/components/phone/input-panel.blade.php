...
@component('components.panel',[
    ...
    <div class="phones-container items-container" data-items-count="{{ $phonesCount }}">
        {!! Former::hidden('phones_sync_input',true) !!}
        @for($i=0;$i<10;$i++)
        <div class="phone-container item-container">
            <div class="row">
                {!! Former::hidden('phones_input['.$i.'][weight]',0) !!}
                <div class="col-4">
                    {!! Former::select('phones_input['.$i.'][type]','Type')->options(\App\Options\PhoneType::get('---')) !!}
                </div>
                <div class="col-8">
                    {!! Former::text('phones_input['.$i.'][number]','Number') !!}
                </div>
            </div>
            <hr/>
        </div>
